...
describe('platform-ops singleton', () => {
  ...
  it('get returns the exact ops object passed to set', () => {
    const ops: PlatformOps = { restartApp: jest.fn(), isAppInProgress: jest.fn() };
    setPlatformOps(ops);
    expect(getPlatformOps()).toBe(ops);
  });

  it('reset clears the wired ops back to null', () => {
    setPlatformOps({ restartApp: jest.fn(), isAppInProgress: jest.fn() });
    resetPlatformOps();
    expect(getPlatformOps()).toBeNull();
  });

  it('setting twice keeps the latest ops', () => {
    const first: PlatformOps = { restartApp: jest.fn(), isAppInProgress: jest.fn() };
    const second: PlatformOps = { restartApp: jest.fn(), isAppInProgress: jest.fn() };
