...
export interface ActivityEntry {
  id: string;
  action: 'deploy' | 'git-deploy' | 'start' | 'stop' | 'restart' | 'delete' | 'login' | 'signup' | 'redeploy' | 'migrate-runtime' | 'suspend' | 'unsuspend' | 'login_mfa_challenge' | 'login_mfa_ok' | 'mfa_enabled' | 'mfa_disabled';
  userId?: string;
  username?: string;
  appName?: string;
  detail?: string;
