...
export interface AppState {
  ...
  /**
   * When someone last OPENED this app through the access gate, and who has
   * done so recently (DROP-152 AC3).
   *
   * Held in AppState rather than derived from the access log, deliberately.
   * The log is pruned by `logRetentionDays` — 14 by default and settable to 1
   * — so an app nobody had opened in fifteen days would read identically to
   * one nobody had EVER opened. That is precisely the distinction a stale-app
   * review exists to make, so the estate view cannot be built on a store with
   * an operator-tunable amnesia window. The log stays the evidence trail; this
   * is the summary.
   *
   * `AppStateManager` already debounces its writes through `scheduleSave`,
   * which is what makes a per-open update affordable here and unaffordable in
   * `AppConfig` (rewritten in full on every write, and the source of truth for
   * port assignments).
   */
  lastOpenedAt?: string;
  /**
   * A small, bounded set of the users who most recently opened this app.
   *
   * Bounded because it is rendered to a decision-maker, not queried: the
   * question the estate view answers is "is anyone still using this, and
   * roughly who", and an unbounded list answers it no better while growing
   * without limit in a file read on every boot.
   */
...
/** How many recent openers the estate view carries per app. */
const MAX_RECENT_OPENERS = 10;
...
export class AppStateManager {
  ...
  /**
   * Record that someone opened this app through the access gate.
   *
   * Called from the verify hop, which runs once per HTTP REQUEST — every asset
   * of every page — so this is deliberately cheap and deliberately lossy: the
   * timestamp is rounded to the minute and a repeat within the same minute is
   * dropped without touching state at all. Without that, one page load would
   * queue dozens of saves for a value nobody reads at that resolution.
   */
  async recordAppOpened(name: string, userId: string, username: string): Promise<void> {
    const app = this.apps.get(name);
    if (!app) return;

    // `Date.now()` rather than `new Date()`: same value, and the one a test can
    // move — `jest.spyOn(Date, 'now')` does not affect the constructor.
    const minute = new Date(Math.floor(Date.now() / 60_000) * 60_000).toISOString();
    const existing = app.recentOpeners ?? [];
    const already = existing.find(o => o.userId === userId && o.at === minute);
    if (already) return;

    const recentOpeners = [
      { userId, username, at: minute },
      ...existing.filter(o => o.userId !== userId),
    ].slice(0, MAX_RECENT_OPENERS);

    await this.updateApp(name, { lastOpenedAt: minute, recentOpeners });
  }
