 ...
 *
 * It answers two questions that were previously conflated or not asked at all:
 *
 * 1. **Is this the first load?** Every hook set `loading = true` on *every*
 *    poll, not just the first. `AppsPage` gates its empty-state card on
 ...
 *    went blank and back roughly once a second. "First load" and "background
 *    refresh" are different questions and need separate answers.
 *
 ...
 *    fresh data — or clear a newer error — with stale data.
 *
 ...
 * is not testable there and this repo does not carry jsdom/RTL.
 */
...
export interface PollTracker {
  ...
  /** True until the first response has been applied — this drives `loading`. */
  isFirstLoad(): boolean;
  /** Requests begun but not yet settled — this drives `refreshing`. */
  inFlight(): number;
}
...
export function createPollTracker(): PollTracker {
  ...
  return {
    ...
    settle(ticket) {
      // Counted even when the response loses: the request *did* finish, so it
      // is no longer in flight. Forgetting this pins `refreshing` on forever.
      settled++;
